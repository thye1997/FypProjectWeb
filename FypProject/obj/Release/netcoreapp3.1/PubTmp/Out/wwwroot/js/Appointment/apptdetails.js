...
$('#medPrescriptionForm').submit(function (e) {
    ...
    console.log("med type:=> " + medType + "med name:=> " + medName + "med descr:=> " + desc);
    apptResult.medType = medType;
    apptResult.medName = medName;
    apptResult.description = desc;
    var apptresult = {
        Id: medicineId,
        description: desc
    }
...
function GetMedicineList(Type) {
    $.ajax({
        ...
        async: false,
        success: function (response) {
            PopulateMedicineList(response.list)
        }
    });
}
