...
function LoadUserList() {
    $.ajax({
        ...
        success: function (response) {
            ...
            userLists = response.userList;
            InitUserList(response.userList);
        }
    });
}
function ApptDetails(choice) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            InitServiceList(response.service);
        }
    });
    if (choice != 0) {
...
function InitUserList(userList) {
    for (i = 0; i < userList.length; i++) {
        $('#userDataList').append('<option value="' + userList[i].fullName + '" data-val="' + userList[i].id + '">' + userList[i].fullName + '</option>');
    }
}
...
function LoadApptList() {
    ...
    apptListTable= $('#appt-list-table').DataTable({
        ...
        columns: [
            ...
            {
                "data": "apptType",
                render: function (apptType) {
                    if (apptType == "WalkIn")
                        return "Walk-In";
                    else return apptType;
                }
            },
            { "data": "status" },
            {
